...
func _steal_player_inventory():
	...
	var player_ref = Globals.get_player()
	self.player_inventory_ref = player_ref.get_node("Inventory")
	player_ref.remove_child(self.player_inventory_ref)
...
func _return_player_inventory():
	...
	player_ref.add_child(self.player_inventory_ref)
